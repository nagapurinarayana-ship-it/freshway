(()=>{
  ...
  async function enhanceCash(force=false){const root=document.getElementById('cashSummary');if(!root||(!force&&root.dataset.cashEnhanced==='1'))return;try{const r=await api('/admin/reports?range=today');const d=await api('/admin/orders?page=1&limit=50&payment=Collected&sort=newest');const today=istDay(new Date());const collected=(d.orders||[]).filter(o=>o.status!=='Cancelled'&&istDay(o.paymentCollectedAt||o.updatedAt||o.createdAt)===today);const collectedTotal=collected.reduce((sum,o)=>sum+Number(o.total||0),0);const collectedOrders=collected.length;root.dataset.cashEnhanced='1';root.innerHTML=`<div class="cash-summary-grid"><article class="report"><span>Cash pending</span><strong>${money2(r.cashPending)}</strong><small>${r.cashPendingOrders} orders still outstanding</small></article><article class="report"><span>Cash collected</span><strong>${money2(collectedTotal)}</strong><small>${collectedOrders} collections today</small></article></div><div class="cash-collected-total"><span>Collected today</span><strong>${money2(collectedTotal)}</strong></div>`;const list=document.getElementById('cashList');if(list){list.innerHTML=collected.length?`<div class="section-head"><div><span class="eyebrow">COLLECTED</span><h2>Cash collected today</h2><p>${collectedOrders} orders · ${money2(collectedTotal)} total</p></div></div>`+collected.map(orderCardForCash).join(''):'<div class="empty">No cash collected today.</div>'}}catch(e){root.dataset.cashEnhanced='';toast(e.message)}}
