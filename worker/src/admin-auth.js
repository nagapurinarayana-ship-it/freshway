...
async function sign(env) {
  ...
  const signature = await crypto.subtle.sign('HMAC', await key(env), enc(payload));
...
async function validSession(request, env) {
  ...
  if (!raw) return false;
  const [issuedRaw, signatureHex] = raw.split('.');
...
async function rateLimit(env, request) {
  ...
  await env.DB.prepare(`INSERT INTO auth_rate_limits (key,window_start,count) VALUES (?,?,1)
    ...
    .bind(keyName, start, start, start).run();
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/admin/logout' && request.method === 'POST') {
      const res = response({ ok: true }, 200, env);
      const headers = new Headers(res.headers);
