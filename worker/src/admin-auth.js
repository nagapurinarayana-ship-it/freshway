...
async function validSession(request, env) {
  ...
  if (!raw) return false;
  const [issuedRaw, signatureHex, ...extra] = raw.split('.');
  if (extra.length) return false;
  const issued = Number(issuedRaw);
  if (!Number.isSafeInteger(issued) || !/^[0-9a-f]{64}$/i.test(signatureHex || '')) return false;
  const now = Math.floor(Date.now() / 1000);
  if (issued < now - MAX_AGE || issued > now + 60) return false;
  try {
    const expected = new Uint8Array(await crypto.subtle.sign('HMAC', await key(env), enc(issuedRaw)));
    const actual = new Uint8Array((signatureHex.match(/../g) || []).map(x => parseInt(x, 16)));
...
async function rateLimit(env, request) {
  const keyName = `admin:login:${ip(request)}`;
  const start = Math.floor(Date.now() / 1000 / WINDOW) * WINDOW;
  await env.DB.prepare(`INSERT INTO auth_rate_limits (key,window_start,count) VALUES (?,?,1) ON CONFLICT(key) DO UPDATE SET count=CASE WHEN auth_rate_limits.window_start=? THEN auth_rate_limits.count+1 ELSE 1 END, window_start=?, updated_at=CURRENT_TIMESTAMP`).bind(keyName, start, start, start).run();
...
export default { async fetch(request, env, ctx) {
  ...
  if (url.pathname === '/api/admin/logout' && request.method === 'POST') {
    const res = response({ ok: true }, 200, env); const headers = new Headers(res.headers); headers.append('Set-Cookie', `${COOKIE}=; Max-Age=0; Path=/; HttpOnly; Secure; SameSite=Lax`); return new Response(res.body, { status: res.status, headers });
  }
  if (url.pathname.startsWith('/api/admin/') || url.pathname === '/api/notifications/broadcast') {
    if (!(await validSession(request, env))) return response({ error: 'Unauthorized' }, 401, env, { 'www-authenticate': 'Session' });
    const headers = new Headers(request.headers);
    headers.set('Authorization', `Bearer ${String(env.ADMIN_TOKEN || '').trim()}`);
    headers.delete('X-Freshway-Admin-Token');
