...
async function registerCustomer(env, payload) {
  ...
  await env.DB.prepare(`INSERT INTO customers (id,name,phone,whatsapp_opt_in,updated_at)
    ...
    .bind(String(payload.id).slice(0, 100), String(payload.name || '').slice(0, 100), phone, payload.whatsappOptIn ? 1 : 0).run();
  return { ok: true };
}
...
async function createOrder(env, payload) {
  ...
  return { id, customerId, total, payment: 'Pending', status: 'Ordered', createdAt, updatedAt: createdAt, address: { ...address, pincode: String(address.pincode) }, items };
}

async function customerOrders(env, customerId) {
  if (!customerId) throw new Error('Customer id is required.');
  const { results: rows } = await env.DB.prepare(`SELECT o.id,o.customer_id,o.customer_name,o.customer_phone,o.total,o.payment_status,o.delivery_status,o.created_at,o.updated_at,o.payment_collected_at,
    a.house,a.area,a.city,a.pincode,a.landmark,a.note,
    oi.product_id,oi.name,oi.unit,oi.qty,oi.price,oi.line_total
    FROM orders o JOIN addresses a ON a.id=o.address_id JOIN order_items oi ON oi.order_id=o.id
    WHERE o.customer_id=? ORDER BY o.created_at DESC, oi.id ASC`).bind(customerId).all();
  ...
  for (const r of rows) {
    if (!map.has(r.id)) {
      map.set(r.id, {
        ...
        address: { house: r.house, area: r.area, city: r.city, pincode: r.pincode, landmark: r.landmark, note: r.note },
        items: [],
        total: r.total,
        ...
        paymentCollectedAt: r.payment_collected_at
      });
    }
    map.get(r.id).items.push({ id: r.product_id, name: r.name, unit: r.unit, qty: r.qty, price: r.price, lineTotal: r.line_total });
  }
  return [...map.values()];
}

async function adminOrders(env) {
  const { results } = await env.DB.prepare(`SELECT o.id,o.customer_id,o.customer_name,o.customer_phone,o.total,o.payment_status,o.delivery_status,o.created_at,o.updated_at,o.payment_collected_at,
    a.house,a.area,a.city,a.pincode,a.landmark,a.note,
    oi.product_id,oi.name,oi.unit,oi.qty,oi.price,oi.line_total
    FROM orders o JOIN addresses a ON a.id=o.address_id JOIN order_items oi ON oi.order_id=o.id
    ORDER BY o.created_at DESC, oi.id ASC`).all();
  return groupOrders(results || []);
}

async function updateOrder(env, id, payload) {
  const allowedStatus = ['Ordered', 'Processing', 'Delivered', 'Cancelled'];
  const allowedPayment = ['Pending', 'Collected'];
...
async function addProduct(env, payload) {
  const id = String(payload.id || `p-${Date.now()}-${Math.random().toString(36).slice(2,6)}`).slice(0, 100);
  ...
  await env.DB.prepare(`INSERT INTO products (id,name,unit,price,emoji,active) VALUES (?,?,?,?,?,1)`).bind(id,name,unit,Math.round(price),emoji).run();
  return { id, name, unit, price: Math.round(price), emoji, active: 1 };
}
...
async function broadcastPush(env, payload) {
  ...
  let success=0, failure=0; const dead=[];
  await Promise.all((results || []).map(async row => {
    try { await webpush.sendNotification({endpoint:row.endpoint,keys:{p256dh:row.p256dh,auth:row.auth}},JSON.stringify({title:payload.title||'FreshWay',body:payload.body||'You have a new FreshWay update.',url:payload.url||'/',tag:payload.tag||`freshway-${Date.now()}`})); success++; }
    catch(error) { failure++; const status=error?.statusCode||0; if(status===404||status===410) dead.push(row.endpoint); }
  }));
  ...
  await env.DB.prepare('INSERT INTO notification_log (channel,title,body,recipient_count,success_count,failure_count) VALUES (?,?,?,?,?,?)').bind('app',payload.title||'FreshWay',payload.body||'',results?.length||0,success,failure).run();
...
async function broadcastWhatsApp(env, payload) {
  ...
  await env.DB.prepare('INSERT INTO notification_log (channel,title,body,recipient_count,success_count,failure_count) VALUES (?,?,?,?,?,?)').bind('whatsapp',payload.templateName,payload.body||'',results?.length||0,success,failure).run();
...
export default {
  async fetch(request, env) {
    ...
    if(request.method==='OPTIONS') return new Response(null,{status:204,headers:{'access-control-allow-origin':cors,'access-control-allow-methods':'GET,POST,PATCH,OPTIONS','access-control-allow-headers':'Content-Type,Authorization'}});
