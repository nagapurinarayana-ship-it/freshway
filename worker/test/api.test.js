...
test('customer logout clears the session cookie', async () => { const response = await request('/api/auth/logout', { method: 'POST' }, secureHandler); assert.equal(response.status, 200); assert.deepEqual(await jsonResponse(response), { ok: true }); const cookie = response.headers.get('set-cookie') || ''; assert.match(cookie, /freshway-customer-session=/); assert.match(cookie, /Max-Age=0/); assert.match(cookie, /HttpOnly/); assert.match(cookie, /Secure/); assert.match(cookie, /SameSite=Lax/); });
...
test('customer auth UI no longer uses browser prompt and exposes OTP controls', () => {
  ...
  assert.doesNotMatch(notifications, /window\.prompt\s*\(/);
  assert.match(html, /id="otpModal"/);
  assert.match(html, /id="otpCode"/);
  assert.match(html, /id="otpVerify"/);
  assert.match(html, /id="otpResend"/);
