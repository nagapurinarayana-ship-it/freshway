...
function hardenExisting(){document.querySelectorAll('#modalExactMap').forEach(btn=>{if(btn.dataset.fwHardened)return;btn.dataset.fwHardened='1';const box=btn.closest('.exact-location')||btn.parentElement;const text=box?.textContent||'';const m=text.match(/(-?\d+(?:\.\d+)?),\s*(-?\d+(?:\.\d+)?)/);if(!m||!valid(m[1],m[2])){btn.disabled=true;btn.textContent='Exact map location unavailable';btn.title='This order has no valid saved coordinates.';return}})}
